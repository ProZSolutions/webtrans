...
/**
 * LtripController implements the CRUD actions for Ltrip model.
 */
class LtripController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    {
        $model = new Ltrip();
        $paymodel = new Lpayment();
      
                  

        if ($model->load(Yii::$app->request->post())) {

            $modelVehicle = $this->findVehicle($model->vehicle_id);
            $tripNo = $modelVehicle->vehicle_no;          
            $model->trip_no = date('dmY').$tripNo;
            $model->trip_status = 1;
